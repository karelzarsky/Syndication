...
WHERE s.kind in (1,2) AND (s.LastRecomputeDate IS NULL OR s.LastRecomputeDate < DATEADD(day, -100, getdate())) GROUP BY s.ID, s.LastRecomputeDate HAVING COUNT(1)>10 ORDER BY s.LastRecomputeDate"
            ).ToArray();
        }
...
having count(1)>10 and count (distinct(ticker)) > 3";
        ...
        public static List<int> GetNextArticles(Db ctx)
        {
            return ctx.Articles.Where(a => a.Processed == ProcessState.Waiting).Take(100).Select(a => a.ID).ToList();
        }
    }
}
