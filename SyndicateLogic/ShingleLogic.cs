...
WHERE s.kind in (1,2) AND (s.LastRecomputeDate IS NULL OR s.LastRecomputeDate < DATEADD(day, -1, getdate())) GROUP BY s.ID, s.LastRecomputeDate HAVING COUNT(1)>10 ORDER BY s.LastRecomputeDate"
            ).ToArray();
        }
