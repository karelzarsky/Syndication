...
order by su.ShingleID")
                .ToList()
                .ForEach(s => { TestShingle(s, db); });
            }
        }

        private static void TestShingle(int s, Db db)
        {
            ...
            foreach (var su in sus)
            {
                ...
                Prediction t = Trade(DirectionType.Buy, su.Article.Ticker, su.Article.ReceivedUTC, b.TakeProfitPercent, b.StopLossPercent, b.Comment, b.DaysToTimeout, b.StrategyNr);
                if (t != null)
                {
